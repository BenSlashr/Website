...
const fragmentShader = `
  ...
  uniform vec3 uColor1; // Orange
  uniform vec3 uColor2; // Violet
  uniform vec3 uColor3; // Pink/Magenta
  ...
  vec3 getGradientColor(vec2 uv, float time) {
    ...
    // 12 animated centers with different speeds and patterns - like CodePen
    ...
    // Blend all colors with dynamic intensities and weights
    vec3 color = vec3(0.0);
    ...
    // Add radial overlays with weights
  ...
  void main() {
    ...
    // Touch distortion - strong effect
...
const LiquidGradient = ({ className = '' }: LiquidGradientProps) => {
  ...
  useEffect(() => {
    ...
    scene.background = new THREE.Color(0x0a0e27); // Navy Blue
    ...
    // CodePen Scheme 1 colors - Orange + Navy Blue
    // Orange: #F15A22 = rgb(241, 90, 34)
    // Navy Blue: #0a0e27 = rgb(10, 14, 39)
    const uniforms: Record<string, THREE.IUniform> = {
      ...
      uDarkBase: { value: new THREE.Vector3(0.039, 0.055, 0.153) }, // #0a0e27 - Navy Blue base
    ...
    const handleResize = () => {
      ...
      const viewHeight = Math.abs(cameraRef.current.position.z * Math.tan(fovInRadians / 2) * 2);
      const viewWidth = viewHeight * cameraRef.current.aspect;

      meshRef.current.geometry.dispose();
      meshRef.current.geometry = new THREE.PlaneGeometry(viewWidth, viewHeight, 1, 1);
    };
