...
import {
  getTransversePrestationSlugs,
  getTransversePrestationBySlug,
  Prestation,
...
function generateServiceSchema(prestation: Prestation, slug: string) {
  return {
    ...
    url: `https://www.slashr.fr/ads/${slug}`,
  };
}
  ...
  return {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        position: 3,
        name: prestation.tag,
        item: `https://www.slashr.fr/ads/${slug}`,
      },
    ],
  };
}
...
// Génère toutes les pages Transverses Ads au build (SSG)
export async function generateStaticParams() {
  const slugs = getTransversePrestationSlugs();
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const prestation = getTransversePrestationBySlug(slug);
...
export default async function TransversePrestationPage({ params }: Props) {
  const { slug } = await params;
  const prestation = getTransversePrestationBySlug(slug);
  ...
  return (
    <main className="min-h-screen bg-[#1a1a1a]">
      ...
      {/* Autres expertises - Mode auto : affiche TOUTES les pages Ads */}
      <OtherExpertisesSection
        currentSlug={slug}
        category="transverse"
      />
