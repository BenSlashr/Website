...
import {
  getSeaPrestationSlugs,
  getSeaPrestationBySlug,
  Prestation,
...
function generateServiceSchema(prestation: Prestation, slug: string) {
  return {
    ...
    url: `https://www.slashr.fr/ads/sea/${slug}`,
  };
}
  ...
  return {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        position: 4,
        name: prestation.tag,
        item: `https://www.slashr.fr/ads/sea/${slug}`,
      },
    ],
  };
}
...
// Génère toutes les pages SEA au build (SSG)
export async function generateStaticParams() {
  const slugs = getSeaPrestationSlugs();
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const prestation = getSeaPrestationBySlug(slug);
...
export default async function SeaPrestationPage({ params }: Props) {
  const { slug } = await params;
  const prestation = getSeaPrestationBySlug(slug);
